...
public class ChengFen extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences sp = getSharedPreferences(getResources().getString(R.string.sp_name), MODE_PRIVATE);
		 ...
		 if (isCn) {
			 ...
			 mtTextView.setText("成份捐血");
		}
		 else {
			 mtTextView.setText("Apheresis Donation");
		}
		downLoad();
	...
	private void downLoad() {
		if (isCn) {
			...
			new DownLoadAsy().execute(Content.URL_CHENGFEN_HK)	;
		}
		else {
			new DownLoadAsy().execute(Content.URL_CHENGFEN_EN)	;
		}
	}
	@SuppressLint("NewApi")
      	    ...
      	    try {
      	    	 ...
      	    	 if ("0".equals(code)) {
      	    		 if (isCn) {
      	    			 ...
      	    			 new AlertInfoDialog(ChengFen.this).show();
					}
      	    		 else {
      	    			new AlertInfoDialog_EN(ChengFen.this).show();
					}
					}
      	    	 else {
               ...
               if (isCn) {
      	    		...
      	    		new Dialog_noInternet(ChengFen.this).show();
				} else {
					new Dialog_noInternet_En(ChengFen.this).show();
				}
